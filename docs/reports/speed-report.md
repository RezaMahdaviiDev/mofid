...
در این بنچمارک، ۵ روش مختلف فنی برای ثبت سفارش خرید در ایزی‌تریدر مقایسه می‌شوند تا سریع‌ترین متد شناسایی شود.
...
- **مدل ۵ (API Direct)**: زمان ۳۶۴ میلی‌ثانیه (در صورت موفقیت authentication). در حال حاضر با خطای 401 مواجه بود و به UI automation fallback می‌کرد، اما اکنون این مشکل برطرف شده است.
- **توصیه**: 
  - برای رقابت‌های میلی‌ثانیه‌ای: **مدل ۴** (۲۰۲ms)
  - برای پایداری و سرعت متعادل: **مدل ۳** (۷۶۳ms)
  - برای آینده (پس از حل کامل مشکل authentication): **مدل ۵** (API) می‌تواند به زیر ۱۰۰ms برسد
...
- **چالش**: نیاز به authentication صحیح (مشکل 401 Unauthorized برطرف شده است)
